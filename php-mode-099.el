...
Ignores php-file patterns option; fixed to expression \"\\.\\(inc\\|php[34]?\\)\""
  :type 'boolean
  :group 'php)
...
(define-derived-mode php-mode c-mode "PHP"
  ...
  (set (make-local-hook 'php-mode-pear-hook)
       '((lambda nil (set (make-local-variable 'tab-width) 4))
         (lambda nil (set (make-local-variable 'c-basic-offset) 4))
         (lambda nil (set (make-local-variable 'c-hanging-comment-ender-p) nil))
         (lambda nil (set (make-local-variable 'indent-tabs-mode) nil))))
...
(if (and php-speedbar-config (symbolp 'speedbar))
    (progn
      (speedbar -1)
      (speedbar-add-supported-extension "\\.\\(inc\\|php[34]?\\)")
      ))
...
(defconst php-keywords
  (eval-when-compile
    (regexp-opt
     ...
     ;; "case", "default", and "goto" get special treatment
     '("and" "as" "break" "continue" "declare" "do" "echo" "else" "elseif"
       ...
       "or" "require" "require_once" "return" "static" "switch" "then"
       "var" "while" "xor") t))
  "PHP keywords.")
...
(defconst php-font-lock-keywords-3
  (append
   ...
   (list
    ...
    '("\\$\\(this\\)" (1 font-lock-constant-face nil nil)) ; "this" as constant
