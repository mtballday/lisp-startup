...
;; To activate CUA-mode, copy the cua-mode.el file into a directory
;; on the load-path and place the following lines in your .emacs:
;;
;;	(require 'cua-mode)
;;	(CUA-mode t)
;;
...
;;; Todo:
;;  
;;  - undo for rectangle operations + tidy needs more testing needed
...
;(defcustom CUA-mode-use-kill-ring t
;  "*If non-nil, cut/copy to and paste from the kill-ring.
;Otherwise, cut/copy to and paste from register C."
;  :type 'boolean
;  :group 'CUA-mode)
...
(defun CUA-register ()
  ...
  CUA-cur-register)
;  (or CUA-cur-register
;      (and (not CUA-mode-use-kill-ring) ?C)))
...
(defun CUA-rect-copy-as-kill (&optional ring)
  (if (CUA-register)
    ...
    (setq CUA-rect-last-killed killed-rectangle)
    (if ring
	(kill-new (mapconcat
...
(defun CUA-cmd-insert-newline-at-global-mark ()
  (interactive)
  (CUA-action-insert-newline-at-global-mark))
...
(defun CUA-action-paste ()
  ...
  (if buffer-read-only
	...
	t)
       (CUA-rect-last-killed
	(CUA-insert-rectangle CUA-rect-last-killed)
...
(defun CUA-action-paste-pop ()
  (if (eq CUA-last-action 'CUA-action-paste-rect)
      (let ((start (mark)) (end (point)))
	(setq CUA-rect-last-killed nil)
	(and (> start end)
	     (setq start (prog1 end (setq end start))))
...
(defun CUA-post-hook ()
  ...
  (condition-case nil
	...
	(if (not (window-minibuffer-p (selected-window)))
	    (setq transient-mark-mode (not CUA-rectangle)
...
(defun CUA-mode-init-maps ()
  ...
  (if (not CUA-overriding-rectangle-map)
	...
	(define-key m [RET]		 'CUA-cmd-rotate-rectangle)
	(define-key m [(control ?m)]	 'CUA-cmd-rotate-rectangle)
  ...
  (if (not CUA-overriding-global-mark-map)
	...
	(define-key m [RET]		    'CUA-cmd-insert-newline-at-global-mark)
	(define-key m [(control ?m)]	    'CUA-cmd-insert-newline-at-global-mark)
...
Optional argument BIND identifies what bindings to add."
  (cond
   ...
   ((eq bind 'CUA) ; default CUA mappings
    ...
    (define-key global-map [(control delete)] 'kill-line)
